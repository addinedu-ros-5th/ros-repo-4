...
class JPSPlanner:
    ...
    def plan_path(self, start, goals):
        ...
        for i in range(len(waypoints) - 1):
            sub_path = self.jps_algorithm(self.world_to_map(waypoints[i]), self.world_to_map(waypoints[i + 1]))
            if not sub_path:
                return []
...
class JPSPathPlanner(Node):
    def __init__(self):
        ...
        map_image_path = os.path.join(package_share_directory, 'maps', 'mfc.pgm')
        map_yaml_path = os.path.join(package_share_directory, 'maps', 'mfc.yaml')
    ...
    def amcl_pose_callback(self, msg):
        ...
        self.get_logger().info(f'Received AMCL pose: {self.current_pose}')

    def target_pose_callback(self, msg):
        self.get_logger().info(f'Received target poses: {msg}')
        self.target_poses = [[pose.position.x, pose.position.y] for pose in msg.poses]
        if self.current_pose:
            self.generate_path()

    def generate_path(self):
        if self.current_pose and self.target_poses:
            start = [self.current_pose.position.x, self.current_pose.position.y]
            self.get_logger().info(f'Starting path generation from: {start} to: {self.target_poses}')
            path = self.jps_planner.plan_path(start, self.target_poses)
            if not path:
            ...
            self.get_logger().info(f'Published planned_path: {[(p.pose.position.x, p.pose.position.y) for p in path_msg.poses]}')
            self.target_poses = []  # Clear target poses after generating path
